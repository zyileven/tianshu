...
check_nvidia_driver() {
    ...
    if ! command -v nvidia-smi &> /dev/null; then
        ...
        log_info "Will deploy in CPU mode (automatic fallback)"
        DEVICE_MODE="cpu"
        return 0  # 不报错，继续部署
    fi
    ...
    if nvidia-smi &> /dev/null 2>&1; then
        ...
        DEVICE_MODE="auto"  # 使用自动模式
    else
        log_warning "NVIDIA driver found but not working"
        log_info "Will deploy in CPU mode"
        DEVICE_MODE="cpu"
    fi
    echo ""
}
...
check_nvidia_container_toolkit() {
    ...
    if docker run --rm --gpus all nvidia/cuda:12.6.2-base-ubuntu24.04 nvidia-smi &> /dev/null 2>&1; then
        ...
        log_warning "NVIDIA Container Toolkit not working"
        log_info "Will fallback to CPU mode"
        DEVICE_MODE="cpu"
        echo ""
...
main() {
    log_info "=========================================="
    log_info "🚀 Deploying Tianshu Unified Version (Offline)"
    log_info "=========================================="
    echo ""
    ...
    if [ ! -f ".env" ]; then
        ...
        if [ -n "$DEVICE_MODE" ]; then
            sed -i.bak "s/DEVICE_MODE=.*/DEVICE_MODE=$DEVICE_MODE/" .env
            rm -f .env.bak
            log_success "DEVICE_MODE set to $DEVICE_MODE"
        fi
    ...
    if [ "$DEVICE_MODE" != "cpu" ]; then
        ...
        if $COMPOSE_CMD -f $COMPOSE_FILE exec -T worker nvidia-smi &> /dev/null; then
            ...
            log_warning "Could not verify GPU (worker may be initializing or using CPU)"
        fi
        echo ""
    else
        log_info "ℹ️  Running in CPU mode (no GPU verification needed)"
        echo ""
    fi
    ...
    if [ "$DEVICE_MODE" = "cpu" ]; then
        log_info "ℹ️  Running in CPU mode"
        echo "     - Processing will be slower than GPU mode"
        echo "     - To enable GPU, install NVIDIA driver and Container Toolkit"
    else
